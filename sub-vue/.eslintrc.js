module.exports = {
  ...
  rules: {
    ...
    // false 使用保留字做属性名时, 只能使用[]方式取属性 e.g [0, {'allowKeywords': false}]
    // allowPattern: 当属性名匹配提供的正则表达式时，允许使用[]方式取值,否则只能用.号取值 e.g [0, {'allowPattern': '^[a-z]+(_[a-z]+)+$'}]
    "dot-notation": 0,
    // 文件末尾强制换行
    "eol-last": 0,
    ...
    // declaration: 强制使用方法声明的方式，function f(){} e.g [0, 'declaration']
    // expression：强制使用方法表达式的方式，var f = function() {} e.g [0, 'expression']
    // allowArrowFunctions: declaration风格中允许箭头函数。 e.g [0, 'declaration', { 'allowArrowFunctions': true }]
    "func-style": 0,
    // 在函数括号内强制执行一致的换行符
    ...
    // 'keyword-spacing': [0, {'beforeColon': true, 'afterColon': true}],
    ...
    "max-depth": [0, 6],
    // 强制一行的最大长度
    "max-len": [1, 000, {ignoreTemplateLiterals: true}],
    // 强制最大行数
    "max-lines": 0,
    // 强制回调函数最大嵌套深度 5层
    ...
    "no-useless-concat": 0,
    // ES0015 会提供默认的类构造函数。因此，没有必要提供一个空构造函数或一个简单地委托给它的父类的构造函数，
    ...
    // 禁止调用parseInt()或Number.parseInt()使用两个参数调用：一个字符串; 和0（二进制），8（八进制）或16（十六进制）的基数选项。
